...
async function createRolesAndUsers(db) {
    const rolesResult = await db.collection('roles').insertMany([
        { slug: 'admin', label: 'Admin', permissions: ['create_user', 'read_user', 'update_user', 'delete_user'] },
        { slug: 'account_manager', label: 'Account Manager', permissions: ['read_user'] },
        { slug: 'operations', label: 'Operations', permissions: ['create_user', 'read_user', 'update_user'] },
        { slug: 'product', label: 'Product', permissions: ['read_user'] }
    ]);
    ...
    const usersResult = await db.collection('users').insertMany([
        ...
        { userId:cuid(),email: 'tom.bourgeois@caramail.fr', firstName: 'Tom', lastName: 'Bourgeois', roleId: roleMap['product'].toString() },
        { userId:cuid(),email: 'elisa.guerin@email.com', firstName: 'Élisa', lastName: 'Guerin', roleId: roleMap['account_manager'].toString() }
    ]);
